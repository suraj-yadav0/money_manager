...
class IncomeTrendChart extends ConsumerStatefulWidget {
  const IncomeTrendChart({super.key});

  @override
  ConsumerState<IncomeTrendChart> createState() => _IncomeTrendChartState();
}

class _IncomeTrendChartState extends ConsumerState<IncomeTrendChart> {
  ({DateTime start, DateTime end})? _range;
  Future<List<_DailyIncome>>? _dailyIncomeFuture;
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<({DateTime start, DateTime end})>(dateRangeProvider, (
      ...
      setState(() {
        _range = next;
        _dailyIncomeFuture = _getDailyIncome(ref, next.start, next.end);
      });
    });

    // Initialize on first build
    final currentRange = ref.read(dateRangeProvider);
    if (_range == null || _dailyIncomeFuture == null) {
      _range = currentRange;
      _dailyIncomeFuture = _getDailyIncome(
        ref,
        currentRange.start,
        currentRange.end,
      );
    ...
    return FutureBuilder<List<_DailyIncome>>(
      future: _dailyIncomeFuture,
        ...
        return Column(
          ...
          children: [
            Padding(
              ...
              child: Text('Income Trend', style: theme.textTheme.titleMedium),
            ),
            Card(
              child: Padding(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      SizedBox(
                        height: 250, // Slightly shorter than spending chart
                          ...
                          LineChartData(
                              ...
                              touchTooltipData: LineTouchTooltipData(
                                getTooltipItems: (spots) {
                                  return spots.map((spot) {
                                    final dailyIncome = data[spot.x.toInt()];
                                    final date = dailyIncome.date;
                                    ...
                                    if (dailyIncome
                                      ...
                                      sb.writeln(''); // Spacer
                                      for (final name
                                          in dailyIncome.topTransactions) {
                                        sb.writeln('• $name');
                                      }
                                      if (dailyIncome.hasMore) {
                                        sb.write('+ more');
                                      }
                                    }
                            ...
                            lineBarsData: [
                              LineChartBarData(
                                ...
                                isCurved: true,
                                curveSmoothness: 0.3,
                                color: Colors.green, // Use green for income
                                barWidth: 3,
  ...
  Future<List<_DailyIncome>> _getDailyIncome(
    WidgetRef ref,
    DateTime start,
    DateTime end,
  ) async {
    ...
    // Get income transactions in range with category info
    ...
    for (final row in allTransactions) {
      ...
      String label = t.note ?? c?.name ?? 'Income';
      if (label.isEmpty) label = c?.name ?? 'Income';
    ...
    final List<_DailyIncome> result = [];
