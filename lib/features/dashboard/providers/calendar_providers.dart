...
final calendarDailySummariesProvider =
    FutureProvider<Map<DateTime, DailySummary>>((ref) async {
      ...
      final transactions = await (db.select(
        db.transactions,
      )..where((t) => t.timestamp.isBetweenValues(firstDay, lastDay))).get();
    ...
    FutureProvider<List<TransactionWithCategory>>((ref) async {
      ...
      final query =
          db.select(db.transactions).join([
            ...
            ..where(
              db.transactions.timestamp.isBetweenValues(startOfDay, endOfDay),
            )
...
final calendarMonthTotalsProvider =
    FutureProvider<({double income, double expenses})>((ref) async {
